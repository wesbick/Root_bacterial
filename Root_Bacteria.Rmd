...
t_shannon <- t.test(npshannon ~ Lineage, data = meta_alpha)
t_shannon
```
...
permbac <- adonis(shared ~ Site * Lineage + Sat_lev, data=meta_alpha, method = "bray")
permbac

```


There is a significant *site* effect and *Lineage* effect. Their interaction is marginally significant.
